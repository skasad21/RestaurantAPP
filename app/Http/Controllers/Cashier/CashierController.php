...
class CashierController extends Controller
{
    ...
    public function getTables(){
        ...
        foreach($tables as $table){
            ...
            $html .=
            '<button class="btn btn-outline-light btn-table" data-id="'.$table->id.'" data-name="'.$table->name.'">
        ...
        foreach($menus as $menu){
            ...
            <div class="col-md-3 text-center">
                <a class="btn btn-outline-secondary btn-menu" data-id="'.$menu->id.'">
                    <img class="img-fluid rounded" src="'.url('/menu_images/'.$menu->image).'">
                    <br>
                    '.$menu->name.'
                    <br>
    ...
    private function getSaleDetails($sale_id){
        ...
        if($showBtnPayment){
            $html .= '<button data-id="'.$sale_id.'" data-totalAmount="'.$sale->total_price.'" class="btn btn-success btn-block btn-payment float-end" data-bs-toggle="modal" data-bs-target="#exampleModal">Payment</button>';
        }else{
            $html .= '<button data-id="'.$sale_id.'" class="btn btn-warning btn-block btn-confirm-order float-end">Confirm Order</button>';
        }
